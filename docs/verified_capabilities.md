...
- proven:
  ...
  - `RPT_4` has a `2026-02-18` live slice with repo-local UI capture and accepted API replay package, including normalized workbook and explicit mismatch log
  - `RPT_23` has a `2026-02-18` runtime-first single-date package with normalized workbook, meta sidecar, and governed parity boundary kept explicit
- limits:
  ...
  - `RPT_4` still has residual replay mismatches and unresolved hidden backend predicate/field-closure gaps, so it remains below full parity closure
  - `RPT_23` keeps `runtime_html` as primary truth; native export/file is secondary audit support only, raw parity failed, and first-column canonicalization remains unresolved
