#!/usr/bin/env python3
"""Build normalized workbook and meta sidecar for RPT_29 from saved repo artifacts."""
...
def build_summary_rows(curated: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {"section": "runtime", "metric": "columns", "value": runtime["columns"], "notes": ""},
        ...
        {"section": "runtime", "metric": "empty_report_observed", "value": runtime["empty_report_observed"], "notes": ""},
        {"section": "runtime", "metric": "totals_amount_due", "value": runtime["totals_row"]["amount_due"], "notes": ""},
    ...
    return [
        {
            ...
            "reason": None,
            "amount_due": None,
            "runtime_support_status": "empty_runtime_slice",
            "notes": "runtime report rendered only totals row on this date",
        }
    ]
...
def build_notes_rows(date: str) -> list[dict[str, Any]]:
    return [
        {"field": "live_report_md", "value": (REPORTS_DIR / f"RPT_29_live_report_{date}.md").read_text()},
        {"field": "problem_log_md", "value": (REPORTS_DIR / f"RPT_29_problem_log_{date}.md").read_text()},
    ]
...
def main() -> None:
    ...
    date = args.date
    curated = read_json(REPORTING_DIR / f"RPT_29_live_probe_{date}.json")

    wb = Workbook()
    ws_summary = wb.active
    ws_summary.title = "RPT_29_summary"
    write_table(ws_summary, 1, build_summary_rows(curated), title="summary")

    ws_norm = wb.create_sheet("RPT_29_normalized")
    write_table(ws_norm, 1, build_normalized_rows(curated), title="normalized_rows")

    ws_warn = wb.create_sheet("RPT_29_warnings")
    ...
    ws_notes = wb.create_sheet("RPT_29_notes")
    write_table(ws_notes, 1, build_notes_rows(date), title="notes")

    for ws in wb.worksheets:
        autofit(ws)

    output_path = Path(args.output) if args.output else EXCEL_DIR / f"RPT_29_{date}_normalized.xlsx"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    wb.save(output_path)
    load_workbook(output_path)

    meta_path = ARTIFACTS_DIR / f"RPT_29_{date}_meta.json"
